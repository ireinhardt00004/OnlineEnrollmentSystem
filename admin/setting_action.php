...
if (isset($_SESSION['accountID'])) {
    if (isset($_POST['submit'])) {
        ...
        $query = "SELECT COUNT(*) as count FROM faculty WHERE email = ? AND accountID = ?";
        ...
        $query = "SELECT COUNT(*) as count FROM faculty WHERE password = ? AND accountID = ?";
        $stmt = $db->prepare($query);
        ...
        if ($result['count'] >= 1) {
             header("Location: ../admin/settings.php?error=Email Already in use.");
            exit(); // Stop further execution
            }
*/
        ...
        if ($stmt->rowCount() >=1) {
            ...
            header("Location: ../admin/settings.php?error=Edit successfully!");
        } else {
            echo "No rows were updated.";
            header("Location: ../admin/settings.php?error=Edit unsuccessful!");
        }
